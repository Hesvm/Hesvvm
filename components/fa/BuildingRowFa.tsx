...
export function BuildingRowFa({ building }: { building: Building }) {
  return (
    <motion.div
      variants={{
      ...
      style={{ direction: 'rtl' }}
    >
      {/* Icon + Name + Tagline (right group in RTL) */}
      <div className="flex min-w-0 flex-1 items-center gap-5">
        ...
        <div className="flex min-w-0 flex-col gap-0.5 text-right">
          <p className="break-words font-persian text-[18px] font-semibold text-[var(--text-strong)]">
            {building.name}
          </p>
          <p className="break-words font-persian text-[15px] text-[var(--text-muted)]">
            {building.tagline}
          </p>
        </div>
      </div>

      {/* Open Button (left group in RTL) */}
      <div className="flex items-center gap-3 mr-auto">
        <a
          href={building.url || building.siteUrl || building.appStoreUrl || '#'}
          target="_blank"
          ...
          aria-label={`باز کردن ${building.name}`}
        >
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ transform: "scaleX(-1)" }}>
