...
type StatusBubbleFaProps = {
  status: Status | null
  preview?: boolean
}

const faLabels: Record<Status['type'], string> = {
  song: 'به‌تازگی گوش می‌دهم',
  movie: 'به‌تازگی تماشا می‌کنم',
  game: 'به‌تازگی بازی می‌کنم',
  book: 'به‌تازگی می‌خوانم',
  manual: 'وضعیت فعلی',
}

export function getFaStatusLabel(status: Status) {
  if (status.type === 'manual') {
    if (status.subtitle === 'Open for') return 'آماده برای همکاری'
    return status.subtitle || faLabels.manual
  }
  return faLabels[status.type]
}

function getFaStatusTitle(status: Status) {
  if (status.type === 'manual' && status.title.toLowerCase() === 'projects') {
    return 'پروژه‌ها'
  }
  return status.title
}
...
function StatusMediaImage({ status, className = '' }: StatusMediaImageProps) {
  ...
  return <span className={fallbackClassName}>{status.title.trim().slice(0, 1) || 'ح'}</span>
}
...
export function StatusBubbleFa({ status, preview = false }: StatusBubbleFaProps) {
  if (!status || !status.title.trim()) return null

  const label = getFaStatusLabel(status)
  const title = getFaStatusTitle(status)
  ...
  const content = (
    <>
      ...
      >
        <g filter="url(#status-bubble-shadow-fa)">
          <path
        ...
        <defs>
          <filter id="status-bubble-shadow-fa" x="0" y="0" width="284.6" height="181.6" filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
      ...
      <span className="status-bubble-object" aria-hidden="true" style={{ right: "16px", left: "auto" }}>
        <StatusMedia status={status} />
      </span>
      <span className="status-bubble-copy" style={{ right: "65px", left: "auto", textAlign: "right" }}>
        <span className="status-bubble-label" style={{ fontFamily: "var(--font-persian)" }}>{label}</span>
        <span className="status-bubble-title" style={{ fontFamily: "var(--font-persian)" }}>{title}</span>
        {status.type === 'manual' && status.subtitle ? (
          <span className="status-bubble-subtitle" style={{ fontFamily: "var(--font-persian)" }}>{label}</span>
        ) : null}
      </span>
    </>
  )
  ...
  return (
    <div className={className} role="status" aria-label={ariaLabel} style={{ direction: "rtl" }}>
      {content}
    </div>
  )
}
